...
ifeq ($(OS),Windows_NT)
	...
	ifndef WINCLISUPPORT
		# gui only application for windows
		LDFLAGS += -mwindows
	endif
...
# Deploy build for windows
windeploy: $(TARGET)
	windeployqt --release --compiler-runtime --no-angle $(TARGET)
	powershell -File ./windows_deploy_deep_dependencies.ps1 $(BIN_DIR)

# Phony targets
.PHONY: all run clean runtest windeploy
